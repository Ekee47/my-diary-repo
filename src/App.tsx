...
function parseReadableDateToISO(day: string, monthName: string, year: string): string | null {
...
export function AIResponseRenderer({ text, onDateClick, allowedDates }: AIResponseRendererProps) {
  // Matches THREE possible date phrasings the AI might use, so no format variant
  // silently falls through unlinked: "17th july 2026" (day-month-year), "july 17, 2026"
  // (month-day-year), and bare ISO "2026-07-17".
  const dateRegex =
    /\b(\d{1,2})(?:st|nd|rd|th)?\s+([a-zA-Z]+)\s+(\d{4})\b|\b([a-zA-Z]+)\s+(\d{1,2})(?:st|nd|rd|th)?,?\s+(\d{4})\b|\b(\d{4})-(\d{2})-(\d{2})\b/gi;
  ...
  while ((match = dateRegex.exec(text)) !== null) {
    ...
    if (match[1] && match[2] && match[3]) {
      isoDate = parseReadableDateToISO(match[1], match[2], match[3]); // day month year
    } else if (match[4] && match[5] && match[6]) {
      isoDate = parseReadableDateToISO(match[5], match[4], match[6]); // month day, year
    } else if (match[7] && match[8] && match[9]) {
      isoDate = `${match[7]}-${match[8]}-${match[9]}`; // already ISO
    }
...
function createSearchIndexContentHash(entry: Pick<DiaryEntry, "date" | "title" | "bodyHtml"> & { dailyWin?: string }) {
  const source = JSON.stringify({
    date: entry.date,
    title: entry.title,
    body: entry.bodyHtml.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim(),
...
function hasWrittenContent(bodyHtml: string) {
  return bodyHtml.replace(/<[^>]+>/g, "").replace(/&nbsp;/g, " ").trim().length > 0;
}

function needsSearchIndex(entry: DiaryEntry) {
  if (!hasWrittenContent(entry.bodyHtml)) return false; // nothing to index
  return !hasCurrentSearchIndex(entry);
}

function createSearchIndexingState(entries: DiaryEntry[]): SearchIndexingState {
  const indexable = entries.filter((e) => hasWrittenContent(e.bodyHtml));
  const total = indexable.length;
  const indexed = indexable.filter(hasCurrentSearchIndex).length;
  const failed = indexable.filter((e) => needsSearchIndex(e) && e.aiSearchIndexStatus === "failed").length;
  ...
  return {
    ...
    message:
      failed > 0
        ? `${failed} ${failed === 1 ? "entry needs" : "entries need"} retry`
        : pending > 0
          ? `${pending} ${pending === 1 ? "entry" : "entries"} waiting to index`
          : "All entries indexed",
  };
}
...
export default function App() {
  ...
  const [searchIndexingState, setSearchIndexingState] = useState<SearchIndexingState>(() =>
    createSearchIndexingState([]),
  );
  ...
  /** Reactive indexer: runs whenever entries change (new saves, edits, or a manual
          ...
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400">{indexingState.message}</p>
                </div>
              </div>
              {indexingState.failed > 0 ? (
                <button
            ...
            <div className="mt-3 h-2 overflow-hidden rounded-full border border-white/10 bg-black/40">
              <div
                ...
                style={{
                  width: `${indexingState.total > 0 ? Math.round((indexingState.indexed / indexingState.total) * 100) : 100}%`,
                }}
              />
            </div>
          </div>
